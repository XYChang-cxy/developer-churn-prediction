...
def darts_rnnmodel(repo_id,time_index, data_frame,col_names, divide_point=-12, predict_length=12,
    ...
    if variate_mode==0:  # 单变量预测
        ...
        # model.fit(uni_series, verbose=True)
        ...
        his_preds = model.historical_forecasts(
            train_multi,
            start=input_chunk_length,
            forecast_horizon=6,
            retrain=False,
        ...
        if scalered:
            his_preds = transformer1.inverse_transform(his_preds)
            preds = transformer1.inverse_transform(preds)

        his_pred = TimeSeries.from_times_and_values(his_preds.time_index,
                         his_preds.pd_dataframe().iloc[:,-1])
        pred = TimeSeries.from_times_and_values(preds.time_index,
    ...
    target_trend = decomposeSeries(target_series.pd_series(),'STL',period=6)
    all_pred_trend = decomposeSeries(his_pred.pd_series().append(pred.pd_series()[1:]),'STL',period=6)
    his_pred_trend = all_pred_trend[:divide_point-len(target_series)]
    pred_trend = all_pred_trend[divide_point-len(target_series)-1:]
    ...
    # target_trend.plot(label='actual', linewidth=1.5)
    # his_pred_trend.plot(label='in-sample forecast', linewidth=1.5)
    # pred_trend.plot(label='out-sample forecast', linewidth=1.5)
    plt.legend()
    if col_names[0][0]=='n':
